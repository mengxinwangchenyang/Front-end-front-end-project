...
//引入请求函数
import { reqCategory, reqBannerList, reqFloorList,reqTest,firstMenus } from '@/api'
//仓库存储数据的地方
let state = {
...
let mutations = {
    ...
    GETTEST(state,test){
        state.findresult=test;
    },
    //设置一级菜单
    SET_FIRST_MENUS(state,data){
        state.firstMenus = data
    }
...
let actions = {
    ...
    async getBannerList({ commit, state, dispatch }) {
        ...
        if (result.code == 200) {
            ...
            //     result.data.imgUrl = result.data.imgUrl + now
            // }
            console.log(result.data)
    ...
    async getTest({ commit, state, dispatch }){
        let result= await reqTest();
        commit("GETTEST",result);
    },
    //获取一级菜单
    async firstMenus({commit,state}){
        const res = await firstMenus()
        commit('SET_FIRST_MENUS',res)
    }
};
//仓库计算属性
let getters = {};
